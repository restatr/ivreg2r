...
#
# M-22 fixture re-base (planning/22-spec-matrix.md): this family moved off
# the Card/sim ad hoc data onto the canonical help-file bases (griliches76,
# abdata, nlswork), generated by
# tests/stata-benchmarks/generate-orthog-fixtures.do. `small` is NOT varied
# in the fixtures themselves — Stata's e(cstat)/e(cstatp)/e(cstatdf) are
# invariant to `small` (see that .do file's header) — so every fixture-
# comparison cell below fits both small = FALSE and small = TRUE against the
# same fixture.
...
check_orthog <- function(fit, fixture_path, label) {
  ...
  test_that(paste("Orthog stat matches Stata", label), {
    skip_if(!file.exists(fixture_path), "fixture not found")
...
# ============================================================================
# gril_orthog1: orthog(age mrt), IID/HC1
# Model: lw ~ s + expr + tenure + rns + smsa + factor(year) | iq |
#          med + kww + age + mrt
# ============================================================================

for (spec in list(
  list(vcov = "iid",    suffix = "iid"),
  list(vcov = "robust", suffix = "hc1")
)) {
  fixture_file <- fixture_path(paste0("gril_orthog1_orthog_", spec$suffix, ".csv"))
  for (small in c(FALSE, TRUE)) {
    label <- paste0("gril_orthog1 ", spec$suffix, " small=", small)
    fit <- ivreg2(gril_formula, data = griliches, vcov = spec$vcov,
                  small = small, orthog = c("age", "mrt"))
...
# ============================================================================
# gril_orthogs: orthog(s) — an included regressor, IID
# ============================================================================

for (small in c(FALSE, TRUE)) {
  fixture_file <- fixture_path("gril_orthogs_orthog_iid.csv")
  label <- paste0("gril_orthogs iid small=", small)
  fit <- ivreg2(gril_formula, data = griliches, small = small, orthog = "s")
  check_orthog(fit, fixture_file, label)
}


# ============================================================================
# gril_orthog1_dof: orthog(age mrt) with dofminus(1), IID/HC1
# ============================================================================
...
for (spec in list(
  ...
  fixture_file <- fixture_path(paste0("gril_orthog1_dof_orthog_", spec$suffix, ".csv"))
  for (small in c(FALSE, TRUE)) {
    label <- paste0("gril_orthog1_dof ", spec$suffix, " small=", small)
    fit <- ivreg2(gril_formula, data = griliches, vcov = spec$vcov,
                  small = small, orthog = c("age", "mrt"), dofminus = 1L)
    check_orthog(fit, fixture_file, label)
  }
}
...
# ============================================================================
# gril_orthog2: orthog(med kww age mrt) — restricted model underidentified
# (all 4 excluded IVs tested; restricted model has 0 excluded instruments
# left for 1 endogenous regressor). Stata reports cstat=0.
# ============================================================================

test_that("Orthog with all excluded IVs gives stat=0 (underidentified restricted)", {
  fixture_file <- fixture_path("gril_orthog2_orthog_iid.csv")
  skip_if(!file.exists(fixture_file), "fixture not found")

  for (small in c(FALSE, TRUE)) {
    fit <- ivreg2(gril_formula, data = griliches, small = small,
...
# ============================================================================
# ab_orthog1: orthog(d(w, 2)), one-way cluster(id)
# Model: n ~ 1 | w + k + ys | d(w,1) + d(k,1) + d(ys,1) + d(w,2) + d(k,2) + d(ys,2)
# ============================================================================

for (small in c(FALSE, TRUE)) {
  fixture_file <- fixture_path("ab_orthog1_orthog_cl.csv")
  label <- paste0("ab_orthog1 cl small=", small)
  fit <- ivreg2(abdata_formula, data = abdata, tvar = "year", ivar = "id",
                clusters = ~id, small = small, orthog = "d(w, 2)")
  check_orthog(fit, fixture_file, label)
}


# ============================================================================
# nls_orthog1: orthog(south), two-way cluster(idcode year)
# Model: ln_wage ~ grade + age + ttl_exp | tenure | south + c_city + not_smsa
# ============================================================================

for (small in c(FALSE, TRUE)) {
  fixture_file <- fixture_path("nls_orthog1_orthog_cl2.csv")
  label <- paste0("nls_orthog1 cl2 small=", small)
  fit <- ivreg2(nls_formula, data = nlswork, clusters = ~idcode + year,
                small = small, orthog = "south")
  check_orthog(fit, fixture_file, label)
}
