/*===========================================================================
  ...
  Generates CSV benchmark fixtures for orthogonality (instrument-subset) C-statistic testing (M-22, planning/22-spec-matrix.md), re-based off the Card/sim ad hoc data onto the canonical help-file bases:
    ...
    - nlswork (H104, help.txt:1628): the two-way-cluster wage model, with tenure endogenized against geography instruments as a D5a option-variation (WITH DISCLOSURE -- see the nlswork cell below) to exercise the two-way-cluster orthog code path, since nlswork has no overidentified worked example in the help file.

  `small` variants are deliberately NOT generated for any cell in this file: Stata's e(cstat)/e(cstatp)/e(cstatdf) are invariant to the `small` option (verified byte-identical in the retired card/sim fixtures this family replaces), so a small/non-small pair would just duplicate the same numbers under a different suffix. The R tests fit both small = FALSE and small = TRUE against each fixture here and assert both reproduce the same C-statistic.
...
// --- griliches76: local cache, then BC.edu web fallback (per the H03 base model, help.txt:1138) ---
...
// --- nlswork: local cache, then webuse fallback (per the H104 base model, help.txt:1628) ---
...
// --- D5a: H26 (help.txt:1242) minus gmm2s -- 2SLS IID orthog of two excluded IVs; restricted model stays overidentified (overid df 3 -> 1), nondegenerate difference-of-Sargan ---
...
// --- D5a: H25 (help.txt:1235) minus gmm2s -- orthog of an INCLUDED regressor under 2SLS ---
...
// --- All 4 excluded IVs tested: restricted model underidentified -> Stata reports cstat=0 (guard cell, same role as the retired card_orthog2) ---
...
// --- D5a: H88 minus gmm2s plus orthog -- one-way-cluster orthog; restricted overid 3 -> 2 ---
...
/*===========================================================================
  ...
  D5a WITH DISCLOSURE: nlswork has no overidentified worked example in the help file; this spec endogenizes tenure with geography instruments purely to exercise the two-way-cluster orthog code path on the H104 base model. It is NOT an economically motivated model.
===========================================================================*/
