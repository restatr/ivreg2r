...
Use `fuller = 1` for approximate mean-unbiasedness (to order 1/N) or
`fuller = 4` to target approximate minimum MSE:
...
The null hypothesis is that `motheduc` satisfies the exclusion restriction,
*given that `fatheduc` is a valid instrument*. Failure to reject is consistent
with its validity. Note that removing `motheduc` leaves the model exactly
identified, so the restricted Sargan/J is zero and the C-statistic equals
the full-model overidentification statistic.
...
fit_orthog_robust <- ivreg2(
  lwage ~ exper + expersq | educ | motheduc + fatheduc,
  data = mroz_work, orthog = c("motheduc"), vcov = "robust", small = TRUE
)
