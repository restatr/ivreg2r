...
fit_factor <- ivreg2(lwage ~ exper + expersq + factor(city),
                     data = mroz_work)
coef(fit_factor)
```
...
```{r factor-predict}
new_obs <- data.frame(exper = 10, expersq = 100, city = c(0, 1))
...
| Reduced form (system) | `ivreg2 ..., savefirst` | `ivreg2(..., reduced_form = "system")` |
