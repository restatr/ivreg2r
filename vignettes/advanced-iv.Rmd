...
The tests below follow the escalating order of the help file's testing examples: a joint overidentification test, then a test of a subset of instruments (the help file's Griliches example), then a test of a single regressor's endogeneity, then the empty-endogenous form for an included regressor, and finally the LIML overidentification statistics. Every automatic test lives in `fit$diagnostics`, a named list; `glance()` additionally surfaces the headline tests (weak identification, underidentification, overidentification) as columns. The chunks below read each statistic from `fit$diagnostics`, dropping into `glance()` where a column is available and reads better.
