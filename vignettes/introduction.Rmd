...
cluster-robust inference, weights, and broom integration. For LIML, k-class,
and additional Tier 2 features, see `vignette("advanced-iv")`. For HAC/AC
standard errors, GMM, CUE, partialling, and panel VCE options, see
...
- **IV size**: the critical value such that the *maximal* rejection rate of a
  ...
  10% maximal-size critical value is 16.38.
...
A common rule of thumb is "F > 10 means instruments are strong enough" (Staiger
& Stock, 1997), but the Stock-Yogo (2005) tables give more precise guidance
that accounts for the number of endogenous regressors and instruments, and
distinguish between size distortion and relative bias criteria.
...
- The test has low power when instruments are weak. More generally, failure to
  reject does not guarantee instrument validity --- the test can only detect
  violations of the *overidentifying* restrictions and has no power against
  alternatives where all instruments are invalid in the same direction.
...
- **Shea partial R-sq**: Shea's (1997) partial R-squared, which adjusts for
  intercorrelations among the *endogenous regressors* in the first stage. When
...
`weights` argument. We illustrate with the Mroz data using hours worked as
an analytic weight on a wage equation:
...
`ivreg2r` also supports frequency weights (`weight_type = "fweight"`) and
probability/sampling weights (`weight_type = "pweight"`). The latter forces
robust standard errors regardless of the `vcov` setting. See
`vignette("advanced-iv")` for details.
...
moments. See `vignette("advanced-iv")` for examples and comparisons.
...
| Reduced form | `ivreg2 ..., rf` | `ivreg2(..., reduced_form = "rf")` |
...
| Sigma (weighted) | Weights normalized to sum to N | Same convention | Matches Stata; differs from R's `lm()` by `sqrt(N/sum(w))` |
